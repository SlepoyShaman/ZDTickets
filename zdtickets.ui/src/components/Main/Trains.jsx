...
function Trains() {
    ...
    return (
        <div className="container">
            ...
            <ul className="trains">
                {trains.map(train => {
                    let departureDate = new Date(train.departureTime)
                    return (
                        <li className="trains_info" key={train.id}>
                            <div>Время отправления: <span>{getDate(train.departureTime)}</span></div>
                            <div>Время прибытия: <span>{getDate(train.arrivalTime)}</span></div>
...
function getDate(date) {
    ...
    return `${newDate.toLocaleDateString()} ${newDate.toLocaleTimeString(undefined, {timeStyle: "short"})}`
}
